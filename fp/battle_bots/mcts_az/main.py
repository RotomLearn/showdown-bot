...
from poke_engine import MctsResult, MctsResultAZ
...
PARALLELISM_MULTIPLIER = {6: 1, 5: 1, 4: 2, 3: 3, 2: 3, 1: 3, 0: 3}
# Add a global variable to cache the model in each process
_MODEL_CACHE = None

def select_move_from_mcts_results(mcts_results: list[(MctsResultAZ, float, int)]) -> str:
    final_policy = {}
...
def get_result_from_mcts(
    ...
    if _MODEL_CACHE is None:
        ...
        _MODEL_CACHE = load_model("/usr/local/lib/python3.11/site-packages/poke_engine/pokemon_az_quantized.pt")
        logger.info(f"Process {index}: Model loading took {time.time() - model_load_start:.2f}s")
    
    state_string = poke_engine_state.to_string()
    logger.debug(f"Process {index}: State conversion took {time.time() - start_time:.2f}s")
    
    mcts_start = time.time()
    res = monte_carlo_tree_search_az(poke_engine_state, search_time_ms, _MODEL_CACHE)
    logger.info(f"Process {index}: MCTS took {time.time() - mcts_start:.2f}s for {res.total_visits} iterations")
...
class BattleBot(Battle):
    ...
    def _num_battles_randombattles(self, revealed_pkmn):
        #return int(FoulPlayConfig.parallelism)
    ...
    def find_best_move(self):
        ...
        # Load model once
        # model_id = load_model("/usr/local/lib/python3.11/site-packages/poke_engine/pokemon_az_quantized.pt")
        
        # # Process sequentially for testing
        # mcts_results = []
        # for index, (b, chance) in enumerate(battles):
        #     res = monte_carlo_tree_search_az(
        #         battle_to_poke_engine_state(b),
        #         search_time_per_battle,
        #         model_id
        #     )
        #     logger.info(f"Iterations {index}: {res.total_visits}")
